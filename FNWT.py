...
mirror_frames = [Tidle_crouchM, Tmiss_crouchM, Tcrouch_switch, Thit2_crouchM, Thit2_attacklongM]
# BG
...
class Enemy:
    def __init__(self,tanjie):
        ...
        self.speed = 10 + random.randint(-5,5)
...
class Player:
        ...
        if keydown == True and self.can_attack == True:
            ...
            if self.actions[0] == Thit2:
                ...
                if (direction == "left" and self.looking_left == True) or (direction == "right" and self.looking_left == False): 
                    self.actions.append(Thit2_attacklong) # no mirror
                    self.actions.append(Thit2_attacklong) # no mirror
                if (direction == "right" and self.looking_left == True) or (direction == "left" and self.looking_left == False): 
                    ...
                    self.actions.append(Thit2_attacklongM) # when mirror
                    
                #self.actions.append(Tcrouch)
                attacklong()
                print(f"direction: {direction} while looking left: {self.looking_left}")
    ...
    def draw(self):        
        ...
        if len(self.actions) > 1: # if there is an appended action, 
            ...
            if self.frameduration == 0: # if countdown reaches zero,
                ...
                if self.actions[0] == Thit2_crouchM:
                    if self.looking_left == False: self.velocity = (self.speed/2) # going right    
                    else: self.velocity = -(self.speed/2) # going left
                #recovery
                if self.actions[0] == Tmiss_crouchM:
                    if self.looking_left == False: self.velocity = -(self.speed/2)    
                    else: self.velocity = (self.speed/2)
        ...
        self.y = constant_y + abs(self.velocity)
        self.velocity *= 0.9
        ...
        center = ((WIDTH/2 - self.actions[0].get_width()/2) + self.velocity, self.y - self.actions[0].get_height()/2)
